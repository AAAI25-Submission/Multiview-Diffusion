...
class TrainerBase(nn.Module):
    def __init__(self,
        ...
        if self.epoches is None:
            raise ValueError("Expected epoches but got None")

        self.train_loader = train_loader
        if self.train_loader is None:
            raise ValueError("Expected train_loader but got None")

        self.optimizer = optimizer
        if self.optimizer is None:
            raise ValueError("Expected optimizer but got None")

        self.device = device
        if self.device is None:
            raise ValueError("Please provide the device to run the model on")
        ...
        if IFEarlyStopping:
            if "patience" in kwargs.keys():
                ...
                raise ValueError("Patience is required for early stopping")
        ...
        if IFadjust_learning_rate:
            if "types" in kwargs.keys():
                ...
                raise ValueError("Learning rate adjustment strategy mast be choosen in {type1 or type2}")
    ...
    def adjust_learning_rate(self, epoch, learning_rate):
        if self.types == 'type1':
            ...
            raise ValueError("Please select the learning rate tuning strategy parameter TYPES from {{0}or{1}}".format("type1", "type2"))
...
class SimpleDiffusionTrainer(TrainerBase):
    def __init__(self,
        ...
        if "timesteps" in kwargs.keys():
            ...
            raise ValueError("Expected timesteps but got None")
